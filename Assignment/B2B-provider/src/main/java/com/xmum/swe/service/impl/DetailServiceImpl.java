...
public class DetailServiceImpl implements DetailService {
    ...
    @Override
    public Map<String, Object> insertItem(DetailInsertVO itemVO, MultipartFile multipartFile) {
        //Layer 1
        if(ObjectUtil.isNotNull(multipartFile)){
            itemVO.setFileName(multipartFile.getOriginalFilename());
        }
        try {
            productService.getVisitorById(itemVO.getVIdFk());    //check if the foreign key exists
        ...
        BeanUtils.copyProperties(itemVO, detailBO, "map");
        Timestamp curTime = SpookifyTimeStamp.getInstance().getTimeStamp();
        detailBO.setIId(nextId);
        detailBO.setItCreate(curTime);
        detailBO.setItModified(curTime);
        detailBO.setStatus("Submit");
        detailBO.setOpType("Insert");
        if(ObjectUtil.isNotNull(multipartFile)){
            try {
                detailBO.setFile(multipartFile.getBytes());
                detailBO.setFileName(multipartFile.getOriginalFilename());
            } catch (IOException e) {
    ...
    @Override
    public Map<String, Object> modifyItem(DetailModifyVO itemVO, MultipartFile multipartFile) throws IOException {
        //Layer 1
        ...
        if(ObjectUtil.isNotNull(multipartFile)) detailBO.setFile(newFileData);
        else detailBO.setFile(preDO.getFile());
        //Layer 3
        DetailDO detailDO = new DetailDO();
        BeanUtils.copyProperties(detailBO, detailDO);
